...
import { type Quotation, type QuotationHistory } from '@/lib/types';
import { Clock, User } from 'lucide-react';
import { Timestamp } from 'firebase/firestore';

type QuotationHistoryDialogProps = {
...
export function QuotationHistoryDialog({ quotation, isOpen, onClose }: QuotationHistoryDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>History for Quotation {quotation.quotationId}</DialogTitle>
          <DialogDescription>
            A complete log of all changes made to this quotation.
          </DialogDescription>
        </DialogHeader>
                ...
                {sortedHistory.map((entry, index) => {
                 ...
                 {sortedHistory.length === 0 && (
                    <div className="text-center text-muted-foreground p-10">
                        No history recorded for this quotation.
                    </div>
                )}
            </div>
        </ScrollArea>
