...
export function JobSheetPrintDialog({ jobSheet, isOpen, onClose }: JobSheetPrintDialogProps) {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      printWindow.document.write('<html><head><title>Print Job Sheet</title>');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>Print Job Sheet - {jobSheet.jobId}</DialogTitle>
        </DialogHeader>
        <div className="py-4 bg-gray-200 overflow-y-auto max-h-[70vh]">
            <div ref={viewRef}>
              <JobSheetView jobSheet={jobSheet} hideTotals={true} />
            </div>
        </div>
