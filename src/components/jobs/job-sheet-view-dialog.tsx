...
export function JobSheetViewDialog({ jobSheet, isOpen, onClose, onCreateInvoice }: JobSheetViewDialogProps) {
  ...
  const handleSavePdf = async () => {
    ...
    try {
      ...
      await exportToPdfA4(target, `JobSheet-${jobSheet.jobId}.pdf`);
    } catch (err) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          <DialogTitle>Job Sheet Details - {jobSheet.jobId}</DialogTitle>
        </DialogHeader>
        <div className="py-4 bg-gray-200 overflow-y-auto max-h-[70vh]">
            <div ref={viewRef}>
              <JobSheetView jobSheet={jobSheet} />
            </div>
        </div>
